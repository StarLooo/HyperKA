...
parser.add_argument('--onto_layer_num', type=int, default=3)  # instance gcn层数
...
parser.add_argument('--triple_neg_nums', type=int, default=20)  # 计算triple loss时每个正例对应多少个负例
parser.add_argument('--mapping_neg_nums', type=int, default=20)  # 计算mapping loss时每个正例对应多少个负例
...
parser.add_argument('--combine', type=ast.literal_eval, default=True)  # 是否结合第0层和最后一层的嵌入向量
parser.add_argument('--ent_top_k', type=list, default=[1, 3, 5, 10])  # 应当是选取作为输出的预测列表的下标
